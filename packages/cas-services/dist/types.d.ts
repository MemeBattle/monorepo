...
export type SuccessSignUpData = User & {
    partnerId: string;
};
...
export type SuccessLoggerFunction = (status: number, data: unknown, headers: RawAxiosResponseHeaders | AxiosResponseHeaders, config: AxiosRequestConfig) => void;
...
export type SuccessHealthCheck = SuccessAnswer<{
    commit: string;
}>;
...
export type SuccessGetMe = SuccessAnswer<{
    user: User | TemporaryUser;
}>;
