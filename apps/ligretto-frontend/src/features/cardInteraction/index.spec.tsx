...
describe('CardInteractionProvider', () => {
  it('exposes only the operations owned by each public hook overload', () => {
    ...
    expect(screen.getByTestId('integration-api').textContent).toBe('activeTarget,clearActiveTarget,startPlacing')
    expect(screen.getByTestId('targeted-api').textContent).toBe('isActive,isDimmed,isPlacing,toggleActiveTarget')
  })
