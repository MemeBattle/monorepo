...
export function useCardFocus(target?: CardFocusOptions, deps: DependencyList = []) {
  ...
  const { focusedCard, clearFocus, toggleFocus } = context
  ...
  useEffect(() => {
    ...
    return () => clearFocus(target)
    // The caller-provided dependencies define when the rendered card identity changes.
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [clearFocus, target?.type, target?.type === 'row' ? target.index : undefined, ...deps])
  ...
  return target
    ...
    : { focusedCard, clearFocus: () => clearFocus() }
}
