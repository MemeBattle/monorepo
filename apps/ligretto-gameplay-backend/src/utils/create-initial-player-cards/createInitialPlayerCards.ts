 ...
 */
export const createInitialPlayerCards = (playerId: string): Card[] =>
  shuffleCards(
    [CardColors.red, CardColors.blue, CardColors.green, CardColors.yellow].reduce(
      (cards: Card[], cardColor) => cards.concat(...valuesArray.map((value): Card => ({ value: value + 1, color: cardColor, playerId }))),
      [],
    ),
  )
